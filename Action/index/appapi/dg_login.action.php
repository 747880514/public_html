...
class dg_loginAction extends Action{
	...
	public function pic(){
		...
		$data['reg_btn_img']=INDEX_WEB_URL."View/index/img/appapi/comm/reg_btn_img.png?time=".time();

		$data['bind_btn_img']=INDEX_WEB_URL."View/index/img/appapi/comm/bind_btn_img.png?time=".time();

			$data['forget_btn_img']=INDEX_WEB_URL."View/index/img/appapi/comm/forget_btn_img.png?time=".time();
	...
	public function login(){
		...
		$set=zfun::f_getset("jf_reg,extendreg,blocking_price_endday");	//百里.追加blocking_price_endday
		...
		if(empty($data)){
			...
			// 百里.修改前
			// $userdata = array('extend_id'=>$tgid,'loginname'=>$phone,'phone' => $phone, 'nickname' => $phone,'integral' => $jf_reg, 'reg_time' => time(), 'token' => $token);
			// 百里.修改后
			//首单结束期限
