	...
	public function myHhr(){
		...
		if($is_operator==1){
			...
			foreach($userdata as $k=>$v){
				...
				if(doubleval($v['bili'])==0)continue;

				if($v['operator_lv']!='0'&&$is_hhr=='operator')$uids.=",".$v['id'];

				if($v['is_sqdl']>'0'&&$is_hhr=='agent')$uids.=",".$v['id'];

				if($v['is_sqdl']=='0'&&$v['operator_lv']=='0'&&$is_hhr=='default')$uids.=",".$v['id'];
			...
			foreach($userdata as $k=>$v){
				...
				if($v['tg_lv']==$lv&&$v['bili']>0)$uids.=",".$v['id'];//jj explosion
		...
		$user1=appcomm::f_goods("User",$where,"id,head_img,is_sqdl,nickname,reg_time,operator_lv,yq_all_count,phone","reg_time desc",NULL,20);
		...
		foreach($user1  as $k=>$v){
			...
			$user1[$k]['nickname']=appHhrAction::xphone($v['nickname']);
			...
			if($v['operator_lv']=='1')$user1[$k]['Vname']=$set['operator_name'];//运营商

			if($v['operator_lv']=='2')$user1[$k]['Vname']=$set['operator_name_2'];//联合创始人
