...
class appJdGoodsDetailAction extends Action{
	public static function pass_goods(){
		...
		$jd_data=explode("_",$fnuo_id);
		if(!empty($jd_data[1]))$lable=$jd_data[1];
		if(!empty($jd_data[0]))$data['fnuo_id']=$jd_data[0];
		if(!empty($GLOBALS['goods_yhq_data'])){
			$data['yhq']=1;
			$data['yhq_price']=floatval($GLOBALS['goods_yhq_data']['yhq_price']);
			$data['yhq_span']=$GLOBALS['goods_yhq_data']['yhq_span'];
			$data['yhq_url']=$GLOBALS['goods_yhq_data']['yhq_url'];
		}
	...
	public static function comm_goods($data=array()){
		$fnuo_id=filter_check($data['fnuo_id']);
		...
		$data['yhq_url']=$data['fnuo_url']=INDEX_WEB_URL."?mod=appapi&act=gotojingdong&gid=".$fnuo_id."&yhq_url=".urlencode($data['yhq_url']);
		...
		$data['fnuo_id']=$data['id']=$data['fnuo_id'];
	...
	public function jdIndex(){
		...
		$jd_data=explode("_",$fnuo_id);
		if(!empty($jd_data[1]))$lable=$jd_data[1];
		if(!empty($jd_data[0]))$fnuo_id=$jd_data[0];

		//调用京东获取商品信息
		$data=jingdong::id($fnuo_id);

		$yhqs=jdCommGoodsAction::getlist("",array("gids"=>$fnuo_id),1);//标签接口
		$GLOBALS['goods_yhq_data']=$yhqs=reset($yhqs);
		if(empty($data['id'])){
			$data=($yhqs);
		}

		if(!empty($lable)){
			$lable_data=jdCommGoodsAction::get_lable_list("",array("gid"=>$fnuo_id,"lable"=>$lable),1);//标签接口
			$data=reset($lable_data);
		}
	...
	public function more_share(){
		...
		if(!empty($yhqs)){
			...
			$_POST['yhq_url']=$garr['yhq_url']=$yhqs['yhq_url'];
		}
		...
		foreach($data['goods_img'] as $k=>$v){
			$data['goods_img'][$k]=INDEX_WEB_URL."?mod=appapi&act=appJdGoodsDetail&ctrl=getcode&jd=1&yhq_url=".urlencode($_POST['yhq_url'])."&img=".urlencode($v)."&fnuo_id=".urlencode($_POST['fnuo_id'])."&getGoodsType=".$_POST['getGoodsType']."&token=".$_POST['token'];
		}
	...
	public function share(){
		...
		$data['share_img']=INDEX_WEB_URL."?mod=appapi&act=appJdGoodsDetail&ctrl=getcode&jd=1&yhq_url=".urlencode($_POST['yhq_url'])."&fnuo_id=".urlencode($_POST["fnuo_id"])."&getGoodsType=".$_POST['getGoodsType']."&token=".$_POST['token'];
 ...
 //        );
	 ...
	 * @DateTime 2019-01-18T17:51:17+0800
