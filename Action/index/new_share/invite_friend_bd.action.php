...
class invite_friend_bdAction extends Action{
	...
	public function new_packet(){
		...
		$str="share_inv_title,CustomUnit,xinren_hongbao,is_openbd,yqzc_tx_onoff,AppDisplayName,fx_detail,fx_middetail,fx_icon,app_sjdl_onoff";
		...
		$this->assign("INDEX_WEB_URL",INDEX_WEB_URL);
		...
		$this->display("invite_friend_bd","new_packet","wap");
	...
	public function goods_detail(){
		...
		$this->display("invite_friend_bd","goods_detail","wap");
	...
	public function old_packet(){
		...
		$set=zfun::f_getset("share_get_str4,share_get_str1,share_get_str2,share_get_str3,share_inv_title,xinren_hongbao,is_openbd,AppDisplayName,fx_detail,fx_middetail,AppLogo,android_url,ios_url");
		...
		if(strpos($_SERVER['HTTP_USER_AGENT'], 'iPhone')||strpos($_SERVER['HTTP_USER_AGENT'], 'iPad')){
			...
			$set['url']= INDEX_WEB_URL . 'Upload/apk/'.$version['name'];

			if(empty($version['name']))$set['url']=$set['android_url'];
		...
		$this->display("invite_friend_bd","old_packet","wap");
	...
	public function questionnaire(){
		...
		$this->display("invite_friend_bd","questionnaire","wap");
	...
	public function succeed(){
		...
		$set=zfun::f_getset("rmb_ico,share_get_str5,share_inv_title,xinren_hongbao,is_openbd,AppDisplayName,fx_detail,fx_middetail,AppLogo");
		...
		$this->display("invite_friend_bd","succeed","wap");
	...
	public function wapyzm1(){//手机注册验证码
		...
		$set1=zfun::f_getset("yqzc_tx_onoff");

		if(empty($set1['yqzc_tx_onoff'])){

			if (empty($_GET['tx_code'])){echo "fun1('请输入图形验证码',0,0);";exit;}

			if(empty($_SESSION['captcha'])){echo "fun1('请输入图形验证码',0,0);";exit;}

			if($_SESSION['captcha']!=md5(strtoupper($_GET['tx_code']).'')){echo "fun1('图形验证码错误',0,0);";exit;}
		...
		if ($user > 0){echo "fun1('您已经是老用户',2,0);";exit;}

		$fndx=$this->getApi('sendDx');

		$end=$fndx->send($msgstr,$phone);

		if (!$end){echo "fun1('发送失败',1,1);";exit;}

		$data=array();

		$data['yzm']=md5(strtoupper($yzm));
	...
	public static function  setCook($data){
		...
		$url=ROOT_PATH."Temp/yzm/".$str.".cache";

		$ht_time=120;

		zfun::wfile($url,json_encode(array("data"=>$c_arr,"end_time"=>time()+$ht_time)));
	...
	public function checkcode(){
		...
		if(file_exists($url)==true){
			...
			$data_al=json_decode(zfun::get($url),true);
		...
		if($data_al['end_time']<time()){echo "fun2('验证码失效',0,0);";exit;}

		if (empty($att['yzm'])){echo "fun2('请先发送验证码',0,0);";exit;}

		if ($att['phone'] != $phone){echo "fun2('手机号不正确',0,0);";exit;}

		if (empty($_GET['password'])){echo "fun2('请输入密码',0,0);";exit;}



		if ($att['yzm'] != ($yzm)) {echo "fun2('验证码错误',0,0);";exit;}
		...
		if ($user > 0){echo "fun2('您已经是老用户',2,0);";exit;}
		...
		$data['dq1']=intval($_GET['dq1']);

		$data['dq2']=intval($_GET['dq2']);

		$data['dq3']=intval($_GET['dq3']);

		$data['dq4']=intval($_GET['dq4']);
	...
	public function getdq(){
		...
		if(empty($_GET['id'])&&$_GET['type']!=1){echo "fun3('NULL',0,0);";exit;}

		if(empty($_GET['type'])){echo "fun3('NULL',0,0);";exit;}
		...
		self::jsecho("fun3",$dq_arr[$type]['dq_type'],$arr,1,$_GET['type']);

		//echo "fun3(".$dq_arr[$type]['dq_type'].",".$arr.",1,".$_GET['type'].");";
	...
	public static function jsecho($funname='',$msg='',$data=array(),$success=0,$type=0){
		...
		echo $funname."(".json_encode($arr).");";exit;
