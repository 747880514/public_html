...
class getyqjlAction extends Action{
	public function index(){
		...
		if(empty($yqcommission01))zfun::fecho("后台满多少佣金没设置");
		$times=time();

		//百里.修改前
		// $user=zfun::f_select("User","reg_time>$time and (next_visit_time<$times ) and extend_id<>0 and is_fan_yqyj=0 and commission>".$yqcommission01,"extend_id,is_sqdl,id,commission",200,0,"id asc");
		//百里.修改后
		...
		foreach($user as $k=>$v){
			...
			if($commission_spread==0){
				zfun::f_update("User","id='".$v['id']."'",array("next_visit_time"=>$time+12*3600));
				continue;
			}
